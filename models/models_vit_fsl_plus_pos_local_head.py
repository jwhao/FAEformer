...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        return xs, xq, xs_patch, xq_patch  #, xs_, xq_
    ...
    def forward(self, x):
        ...
        xs = x[0].reshape(-1,x[0].shape[-1])
        xq = x[1].reshape(-1,x[0].shape[-1])
        xf = torch.cat([xs,xq],dim=0)
        logits = self.forward_head(xf)
        return x[0],x[1],x[2],x[3],logits
